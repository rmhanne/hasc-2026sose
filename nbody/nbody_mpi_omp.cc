...
void acceleration_blocking_omp(int n, double3 *__restrict__ x, double *__restrict__ m,
...
#pragma omp parallel shared(aglobal), firstprivate(n, x, m, masses_per_rank, B, P)
  {
    ...
    if (id<0 || id>=numthreads)
      std::cout << "rank=" << rank << " thread=" << id << " thread id out of range" << std::endl;
...
#pragma omp critical
  ...
  // in the thread parallel only case we are done
  if (P==1) return;
          ...
          if (g(J / B, s) > g(I / B, rank))
            for (int j = J; j < J + B; j++)
              for (int i = I; i < I + B; i++)
              {
                ...
                double factori = m[i] * invfact;
                double factorj = m[j] * invfact;
                aself[i][0] += factorj * d0;
                aself[i][1] += factorj * d1;
                aself[i][2] += factorj * d2;
                aother[j][0] -= factori * d0;
                aother[j][1] -= factori * d1;
                aother[j][2] -= factori * d2;
              }
#pragma omp critical
      {
        for (int i = 0; i < masses_per_rank; i++)
...
void leapfrog(int n, double dt, double3 *__restrict__ x, double3 *__restrict__ v, double *__restrict__ m, double3 *__restrict__ a)
{
  ...
  acceleration_blocking_omp(n, x, m, a);
...
int main(int argc, char **argv)
{
  ...
  else // invalid command line, print usage
  {
    if (rank == 0)
    {
      ...
      std::cout << "nbody_vanilla <basename> <load step> <final step> <every> <numthreads>" << std::endl;
      std::cout << "nbody_vanilla <basename> <nbodies> <timesteps> <timestep> <every> <numthreads>" << std::endl;
    }
    MPI_Finalize();
    return 0;
  }
  ...
  {
    if (rank == 0)
      std::cout << P << "=P is not even and greater 1" << std::endl;
    MPI_Finalize();
    return 0;
  }
