...
// y = Ax, matrix accessed consecutively, reuse on y (A is stored row-major)
...
// y = Ax, matrix accessed consecutively and reuse on y (A is stored column-major)
...
// y = Ax, blocked variant of column-major variant (A accessed consecutively, reuse on x)
void matvec3 (int n, int b, const double* A, const double* x, double* y)
{
  ...
  if (b%B!=0) {
    std::cout << b << " is a multiple of B" << std::endl;
    exit(0);
  }
  for (int i=0; i<n; i++)
    y[i] = 0.0;
...
int main (int argc, char** argv)
{
  ...
  for (int i=48; i<=25000; i*=2) sizes.push_back(i);
