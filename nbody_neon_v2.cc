		...
		for (int J = I + B; J < n; J += B)
			for (int i = I; i < I + B; i += 2)
				for (int j = J; j < J + B; j += 2)
				{
					...
					A0 = vld1q_f64(&(a[j]));				 // x coordinates
					A1 = vld1q_f64(&(a[j + n]));		 // y coordinates
					A2 = vld1q_f64(&(a[j + 2 * n])); // z coordinates
					...
					A0 = vld1q_f64(&(a[i]));				 // x coordinates
					A1 = vld1q_f64(&(a[i + n]));		 // y coordinates
					A2 = vld1q_f64(&(a[i + 2 * n])); // z coordinates
					// update
					A0 = vfmaq_f64(A0, S, D0); // fma
					A1 = vfmaq_f64(A1, S, D1); // fma
					A2 = vfmaq_f64(A2, S, D2); // fma
