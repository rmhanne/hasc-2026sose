  ...
  for (int i=0; i<n; i+=M) // loop over tiles
	...
	// now C_ij is again blocked int 4x(3Q) blocks
	      ...
	      for (int p=0; p<4; ++p)
		{
		  ...
		  CC[p][0].load(&C[s+p][t]); CC[p][1].load(&C[s+p][t+Q]); CC[p][2].load(&C[s+p][t+2*Q]);
		}
		...
		{
		  // load store amortized over M/8 matrix multiplications
		  CC[p][0].store(&C[s+p][t]); CC[p][1].store(&C[s+p][t+Q]); CC[p][2].store(&C[s+p][t+2*Q]);
		}
	    }
}
