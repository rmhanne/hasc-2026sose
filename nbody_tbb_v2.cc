...
void acceleration_blocked(int ibegin, int iend, int n, double3 *__restrict__ x, double *__restrict__ m, std::vector<std::array<double, M>> &a)
{
...
public:
  ...
  {
    acceleration_blocked(r.begin() * B, r.end() * B, n, x, m, a);
  }
...
void leapfrog(int n, double dt, double3 *__restrict__ x, double3 *__restrict__ v, double *__restrict__ m, double3 *__restrict__ a)
{
  ...
  else
  {
    ...
    for (int i = 0; i < n; i += B)
      acceleration(i, i + B, n, x, m, atemp);
    for (int i = 0; i < n; ++i)
      for (int j = 0; j < 3; j++)
        a[i][j] = atemp[i][j];
  }
