 ...
 */
void acceleration_x (int ibegin, int iend, int n, double3* __restrict__ x, double* __restrict__ m, std::vector<std::array<double,M>>& a)
{
...
// works on a range [ibegin,iend) of rows, where
// ibegin, iend are assumed to be multiples of B
void acceleration (int ibegin, int iend, int n, double3* __restrict__ x, double* __restrict__ m, std::vector<std::array<double,M>>& a)
{
...
// iteration range is over block numbers
class AccelerationKernel
{
  int n;
  double3* x;
  double*  m;
  std::vector<std::array<double,M>> a;
  int rank;
public:
  AccelerationKernel (int _n, double3* _x, double* _m)
    : n(_n), x(_x), m(_m), a(_n,{0.0,0.0,0.0,0.0}), rank(0)
  {
    // oneapi::tbb::spin_mutex::scoped_lock lock{mutex};
    // rank = count_workers++;
    // std::cout << "worker " << rank << std::endl;
  }
  AccelerationKernel(AccelerationKernel& other,
		     oneapi::tbb::split)
    : n(other.n), x(other.x), m(other.m),
      a(other.n,{0.0,0.0,0.0,0.0}), rank(0)
  {
    // oneapi::tbb::spin_mutex::scoped_lock lock{mutex};
    // rank = count_workers++;
    // std::cout << "worker " << rank << std::endl;
  }
  void join (const AccelerationKernel& other)
  {
    for (int i=0; i<n; ++i)
      for (int j=0; j<3; j++)
	a[i][j] += other.a[i][j];
  ...
  {
    acceleration(r.begin()*B,r.end()*B,n,x,m,a);
  }
  void get_result (double3* _a)
  {
    for (int i=0; i<n; ++i)
      for (int j=0; j<3; j++)
	_a[i][j] = a[i][j];
...
void leapfrog (int n, double dt, double3* __restrict__ x, double3* __restrict__ v, double* __restrict__ m, double3* __restrict__ a)
{
  ...
  if (useTBB)
    {
      AccelerationKernel akernel(n,x,m);
      oneapi::tbb::parallel_reduce(oneapi::tbb::blocked_range<size_t>(0,n/B),akernel);
      akernel.get_result(a);
    }
  else
    {
      std::vector<std::array<double,M>> atemp(n,{0.0,0.0,0.0,0.0});
      for (int i=0; i<n/B; ++i)
	acceleration(i*B,(i+1)*B,n,x,m,atemp);
      for (int i=0; i<n; ++i)
	for (int j=0; j<3; j++)
	  a[i][j] = atemp[i][j];
    }
