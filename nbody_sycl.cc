 ...
 */
void acceleration (size_t n, double3* x, double* m, double3* a)
{
  // allocate memory on device
  double3* adevice = sycl::malloc_device<double3>(n,Q);
  double3* xdevice = sycl::malloc_device<double3>(n,Q);
  double*  mdevice = sycl::malloc_device<double>(n,Q);

  // submit tasks for transfering x,m
  // Q is an in-order queue!
  Q.submit([&](sycl::handler& h){h.memcpy(xdevice,x,n*sizeof(double3));});
  Q.submit([&](sycl::handler& h){h.memcpy(mdevice,m,n*sizeof(double));});

  // clear acceleration array on device
  ...
  // copy acceleration from device
  Q.submit([&](sycl::handler& h){h.memcpy(a,adevice,n*sizeof(double3));});

  // wait for all tasks to finish
  Q.wait();

  // and free memory on device
  sycl::free(mdevice,Q);  
  sycl::free(xdevice,Q);  
  sycl::free(adevice,Q);  
}
...
int main (int argc, char** argv)
{
  ...
  // report device on which we execute
