...
// this is the function we want to execute as a thread
...
double parallel_norm_pt (double* v, int N)
{
  ...
  // execute as a task
  // this will be double(double*, double*, double)
  ...
  std::packaged_task<TaskType> pt0{norm2}; // ready to be executed
  std::packaged_task<TaskType> pt1{norm2}; // ready to be executed

  // get the future from packed_task
  std::future<double> f0{pt0.get_future()};
  std::future<double> f1{pt1.get_future()};

  // now start the tasks
  std::thread t0{std::move(pt0),v,v+N/2,0.0};
  std::thread t1{std::move(pt1),v+N/2,v+N,0.0};
